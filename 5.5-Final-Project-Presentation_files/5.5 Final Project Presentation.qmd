...
Time spent on unpaid vs paid work
...
PCA space. The first four PCs are defined below. Each PC represents a linear combination of variables that capture the most variation across individuals in this sample (** = most influential).
...
![](../figures/pca_clusters_annotated.png)
:::
...
ggplot(data_long, aes(x = work_type, y = hours, fill = female)) +
  ...
  labs(
    ...
    subtitle = "Initial look at gender differences in working time",
...
```
Women spend similar time on paid work but more on unpaid domestic work
...
-   When controlling for $C_i$, each additional hour spent on unpaid domestic work is associated with spending
    ...
    -   0.79 fewer hours spent on paid work for females.
:::
:::::
...
ggplot(data2010s2FT, aes(x = factor(female, labels = c("Men", "Women")),
  ...
  labs(
    ...
    subtitle = "Initial look at gender differences in wages",
    x = "",
    y = "Monthly Wage",
    fill = "Gender"
  ) +
