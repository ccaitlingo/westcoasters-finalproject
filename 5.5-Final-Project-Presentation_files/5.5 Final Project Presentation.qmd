---
title: "Paid vs. Unpaid Labor by Sex"
author: "The West Coasters: Caitlin Gong, Meredith Hillier, & Leela Khatri"
execute:
  echo: true
format:
  revealjs:
...
## Topic of Interest
  ...
  * Investigate the time tradeoff 
  * Time is a form of capital with an associated opportunity cost... how do individuals allocate their time in their full 24 hour day?
  * We speculate there may be differences across demographic variables, including sex, age, and marital status
...
## EDA: Unpaid Domestic Work by Sex

We can see that over time, the gap between male and female has shrunk. Time spent by females on unpaid domestic work has remained relatively steady, while male time has increased slightly. However, a disparity still remains: even our most current data from 2018 shows that males on average spend 3.10 hours on unpaid domestic work, while females spend 5.26 hours. 
...
## EDA: Average Daily Time Use by Sex
...
The first four PCs are defined below. Each PC represents a linear combination of variables that capture the most variation across individuals in this sample (** = most influential).
...
::: {.column width="30%"}
**PCA 1:** Time spent on childcare, age of youngest child, and
amount of children under 5
...
Left with ~2700 observations in the end
...
We ran two linear regressions models:
$$\text{ACT_WORK} = \beta_0 + \beta_1 \cdot \text{ACT_UNPAID} \times \text{SEX} + \delta \cdot C_i $$
$$\text{WAGELM} = \beta_0 + \beta_1 \cdot \text{ACT_UNPAID} \times \text{SEX} + \delta \cdot C_i $$

  * $C_i$ represents our control variables: individuals' full-time employment status and employment status of partner.
  * Interaction term coefficient $\beta_1$ represents how the relationship between time spent on unpaid domestic work and the outcome variable varies by sex. 
...
```
Women who work full-time spend a similar amount of time on paid work but more time on unpaid domestic work.
...
::: {.column width="50%"}
-   When controlling for $\,C_i\,$ and time spent on unpaid domestic work, there is no statistically significant difference between males and females' time spent at paid work.

-   When controlling for $\,C_i\,$, each additional hour spent on unpaid domestic work is associated with spending
...
**Takeaway**: There is evidence of societal expectations that women continue handling domestic work despite similar paid work obligations.
...
Women on average earn less per month than men, but still need to consider controlling for other variables.
...
::: {.column width="50%"}
-   When controlling for $\,C_i\,$ and time spent on unpaid domestic work, females on average earn $1,517.20 less per month than males.

-   When controlling for $\,C_i\,$, each additional hour spent on unpaid domestic work is associated with earning
...
**Takeaway**: Same societal expectations are showing, where time spent on unpaid domestic work has a larger negative effect on male earnings than female earnings.


## Takeaways!

  * Findings affirm our hypotheses that the time tradeoff (between unpaid domestic work and paid work) does differ by gender, with men more severely affected
  * Women are expected to work the same amount or more, while earning less on average
  * For additional time spent on unpaid domestic work, the sacrifice for paid work and wages is larger for men than it is for women.


## Limitations

  * Diary data is messy and can be inconsistent
  * Is there some selection bias with our sample? Are our filters too strict or not strict enough?
  * Limited to male/female binary and heterosexual relationships

### Future Work

  * Include different demographic variables in regressions, like race and education level
  * Include other time use variables as outcomes, like physical activity and leisure
